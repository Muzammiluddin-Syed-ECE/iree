...
//===----------------------------------------------------------------------===//
// PackedScaledMMA Attributes
//===----------------------------------------------------------------------===//

int64_t PackedScaledMMAAttr::getExpectedNumInputs() const { return 4; }

int64_t PackedScaledMMAAttr::getExpectedNumOutputs() const { return 1; }

int64_t PackedScaledMMAAttr::getBlockSize() const {
  switch (getIntrinsic()) {
  case ScaledMMAIntrinsic::MFMA_SCALE_F32_16x16x128_B32:
  case ScaledMMAIntrinsic::MFMA_SCALE_F32_32x32x64_B32:
    return 32;
  ...
  assert(false &&
         "all cases should've been handled in PackedScaledMMA::getBlockSize()");
  return 0;
}

int64_t PackedScaledMMAAttr::getSubgroupSize() const {
  switch (getIntrinsic()) {
  case ScaledMMAIntrinsic::MFMA_SCALE_F32_16x16x128_B32:
  case ScaledMMAIntrinsic::MFMA_SCALE_F32_32x32x64_B32:
    return 64;
  ...
  assert(false &&
         "all cases should'vee been handled in PackedScaledMMA::getBlockSize()");
  return 0;
}

LogicalResult
PackedScaledMMAAttr::verifyIndexingMaps(ArrayRef<AffineMap> maps) const {
  if (failed(verifyMmaIndexingMaps({maps[0], maps[2], maps[4]}))) {
    return failure();
  }
...
void PackedScaledMMAAttr::getUndistributedTileTypes(
    SmallVectorImpl<VectorType> &results) const {
  MMASingleSubgroupLayout lhsLayout =
      getSingleSubgroupLayout(getIntrinsic(), 0);
  MMASingleSubgroupLayout rhsLayout =
...
void PackedScaledMMAAttr::getDistributedTileTypes(
    SmallVectorImpl<VectorType> &results) const {
  Type lhsType = getLhsElemType();
  Type rhsType = getRhsElemType();
  Type accType = getAccElemType();
...
std::optional<SmallVector<int64_t, 2>>
PackedScaledMMAAttr::getUndistributedTileDimExpansion(int64_t operandIndex,
                                                int64_t dim) const {
  assert(operandIndex <= 4 && "invalid operand index");
  MMASingleSubgroupLayout layout =
      getSingleSubgroupLayout(getIntrinsic(), operandIndex, getColMajor());
...
Attribute PackedScaledMMAAttr::getDistributionMappingKind() const {
  return IREE::GPU::LaneIdAttr::get(getContext(), 0);
}

OpFoldResult PackedScaledMMAAttr::getDistributionWorkerCount(OpBuilder &, Location,
                                                       Operation *) const {
  return getAsIndexOpFoldResult(getContext(), getSubgroupSize());
}

LogicalResult PackedScaledMMAAttr::populateOperandOffsetsSizesStrides(
    OpBuilder &builder, Location loc, uint32_t operandIndex, Value laneId,
    ArrayRef<int64_t> permutation, SmallVectorImpl<OpFoldResult> &offsets,
    SmallVectorImpl<OpFoldResult> &sizes,
    SmallVectorImpl<OpFoldResult> &strides) const {
...
LogicalResult PackedScaledMMAAttr::buildUnderlyingOperations(
    OpBuilder &builder, Location loc, ValueRange inputs, ValueRange outputs,
    SmallVectorImpl<Value> &results) const {
  if (inputs.size() != 4) {
    return failure();
