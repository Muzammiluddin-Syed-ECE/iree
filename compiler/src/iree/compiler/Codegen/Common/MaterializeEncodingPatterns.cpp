...
lowerOpWithEncoding(RewriterBase &rewriter, tensor::EmptyOp emptyOp,
  ...
  if (IREE::Codegen::isIdentityLayout(encodingInfo) ||
      encodingInfo.isSameShapePermutation) {
...
/// Convert set_encoding for same-shape permutations (scale preshuffling).
/// Emits: pack + expand + swizzle transpose + regroup transpose + collapse.
/// The output has the same rank and shape as the input but with permuted
/// element order.
struct PreshuffleSetEncodingOpLowering
    : OpConversionPattern<IREE::Encoding::SetEncodingOp> {
  PreshuffleSetEncodingOpLowering(
      const MaterializeEncodingTypeConverter &typeConverter,
      MLIRContext *context, PatternBenefit benefit = 2)
      : OpConversionPattern<IREE::Encoding::SetEncodingOp>(typeConverter,
                                                           context, benefit) {}
  ...
  matchAndRewrite(IREE::Encoding::SetEncodingOp encodingOp, OpAdaptor adaptor,
    ...
    if (!encodingInfo.isSameShapePermutation || !encodingInfo.swizzle)
      return failure();
    ...
    Type elemType = encodingOp.getSourceType().getElementType();

    // Step 2: Expand shape (tile the inner dims).
    ...
    SmallVector<ReassociationIndices> expandReassociation =
        getReassociationIndices(origRank, swizzle.expandShape());
    auto expandShapeOp = tensor::ExpandShapeOp::create(
        rewriter, loc, expandShapeType, packedValue.value(),
        expandReassociation);

    // Step 3: Swizzle transpose (permute expanded inner dims).
    SmallVector<int64_t> swizzlePerm =
        llvm::to_vector(llvm::seq<int64_t>(0, origRank));
    for (auto perm : swizzle.permutation()) {
      swizzlePerm.push_back(origRank + perm);
    }
    SmallVector<OpFoldResult> swizzleResultDims =
        tensor::getMixedSizes(rewriter, loc, expandShapeOp.getResult());
    applyPermutationToVector(swizzleResultDims, swizzlePerm);
    auto swizzleEmpty =
        tensor::EmptyOp::create(rewriter, loc, swizzleResultDims, elemType);
    auto swizzleTranspose = linalg::TransposeOp::create(
        rewriter, loc, expandShapeOp, swizzleEmpty, swizzlePerm);

    // Step 4: Boundary-split collapse — collapse directly from swizzled order.
    // The swizzle's interleaving places intrinsic dims (iM, iK) innermost,
    // making each thread's numRepeats scale bytes contiguous. A regroup
    // transpose would break this contiguity by separating iM and iK into
    // different collapse groups. Instead, find a numerical boundary in the
    // flat dim order where the left product equals the padded first original
    // dim, split a dim if the boundary falls mid-dimension, and collapse.

    Value swizzleResult = swizzleTranspose->getResult(0);
    auto swizzleResultType =
        cast<RankedTensorType>(swizzleResult.getType());
    ArrayRef<int64_t> fullShape = swizzleResultType.getShape();
    ArrayRef<int64_t> origShape = encodingOp.getSourceType().getShape();

    int64_t tileDim0 = 0;
    for (int i = 0; i < (int)encodingInfo.innerDimsPos.size(); ++i) {
      if (encodingInfo.innerDimsPos[i] == 0) {
        tileDim0 = encodingInfo.innerTileSizes[i];
        break;
      }
    }
    assert(tileDim0 > 0 && "expected inner tile for dim 0");
    int64_t paddedDim0 =
        ((origShape[0] + tileDim0 - 1) / tileDim0) * tileDim0;
    ...
    for (int i = 0, e = fullShape.size(); i < e; ++i) {
      if (product == paddedDim0) {
        splitBoundary = i;
        break;
      }
      int64_t next = product * fullShape[i];
      if (next == paddedDim0) {
        splitBoundary = i + 1;
        break;
      }
      if (next > paddedDim0) {
        assert(paddedDim0 % product == 0 &&
               "padded dim must be divisible by accumulated product");
        splitFactor = paddedDim0 / product;
        assert(fullShape[i] % splitFactor == 0 &&
               "dim must be evenly splittable");
        splitDimIdx = i;
        splitBoundary = i + 1;
        break;
      }
    ...
    assert(splitBoundary >= 0 && "must find a valid split point");

    Value collapseInput = swizzleResult;

    if (splitFactor > 0) {
      SmallVector<int64_t> splitShapeVec;
      SmallVector<ReassociationIndices> splitReassoc;
      int outIdx = 0;
      for (int j = 0, e = fullShape.size(); j < e; ++j) {
        if (j == splitDimIdx) {
          splitShapeVec.push_back(splitFactor);
          splitShapeVec.push_back(fullShape[j] / splitFactor);
          splitReassoc.push_back({outIdx, outIdx + 1});
          outIdx += 2;
        } else {
          splitShapeVec.push_back(fullShape[j]);
          splitReassoc.push_back({outIdx});
          outIdx++;
        }
      }
      auto splitType = RankedTensorType::get(splitShapeVec, elemType);
      collapseInput = tensor::ExpandShapeOp::create(
          rewriter, loc, splitType, swizzleResult, splitReassoc);
    }

    int totalDims =
        cast<RankedTensorType>(collapseInput.getType()).getRank();
    SmallVector<ReassociationIndices> collapseReassociation(origRank);
    for (int j = 0; j < totalDims; ++j) {
      collapseReassociation[j < splitBoundary ? 0 : 1].push_back(j);
    }
    auto collapseOp = tensor::CollapseShapeOp::create(
        rewriter, loc, collapseInput, collapseReassociation);

    // If padding caused the collapsed shape to exceed the original, trim.
    auto collapsedType =
        cast<RankedTensorType>(collapseOp.getResult().getType());
    RankedTensorType targetType =
        encodingOp.getSourceType().clone(encodingOp.getSourceType().getShape());
    if (collapsedType != targetType) {
      SmallVector<OpFoldResult> offsets(origRank, rewriter.getIndexAttr(0));
      SmallVector<OpFoldResult> sizes =
          getMixedValues(targetType.getShape(), {}, rewriter);
      SmallVector<OpFoldResult> strides(origRank, rewriter.getIndexAttr(1));
      auto sliceOp = tensor::ExtractSliceOp::create(
          rewriter, loc, targetType, collapseOp.getResult(), offsets, sizes,
          strides);
      rewriter.replaceOp(encodingOp, sliceOp.getResult());
    } else {
      rewriter.replaceOp(encodingOp, collapseOp.getResult());
    }
    return success();
  }
};

/// No-op lowering for unset_encoding on same-shape permutations. The compute
/// dispatch receives plain 2D preshuffled tensors, so unset_encoding is just
/// replaced by its (already-converted) source.
struct PreshuffleUnsetEncodingOpLowering
    : OpConversionPattern<IREE::Encoding::UnsetEncodingOp> {
  PreshuffleUnsetEncodingOpLowering(
      const MaterializeEncodingTypeConverter &typeConverter,
      MLIRContext *context, PatternBenefit benefit = 2)
      : OpConversionPattern<IREE::Encoding::UnsetEncodingOp>(typeConverter,
                                                             context,
                                                             benefit) {}
  ...
  matchAndRewrite(IREE::Encoding::UnsetEncodingOp unsetEncodingOp,
    ...
    auto converter = getTypeConverter<MaterializeEncodingTypeConverter>();
    MaterializeEncodingInfo encodingInfo =
        converter->getEncodingInfo(unsetEncodingOp.getSourceType());
...
void populateMaterializeEncodingPatterns(
  ...
  patterns.insert<PreshuffleSetEncodingOpLowering,
                  PreshuffleUnsetEncodingOpLowering>(typeConverter, context);
