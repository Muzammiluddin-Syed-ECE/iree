...
// RUN: iree-opt --pass-pipeline="builtin.module(iree-dispatch-creation-hoist-encoding-ops,cse)" --iree-gpu-partial-dt-scaled-mma --split-input-file %s | FileCheck %s --check-prefix=PDT
...
// On gfx950, only scale operands (indices 2, 3) of scaled_matmul are hoisted
// out of the dispatch. Data operands (indices 0, 1) stay inside.
...
// Without the flag, all set_encodings are hoisted out of the dispatch.
...
// With --iree-gpu-partial-dt-scaled-mma: scale set_encodings (operands 2, 3)
